...
// Helper function to format event type - FIXED: Removed duplicate properties
  ...
  useEffect(() => {
    ...
    const initializeDashboard = async () => {
      try {
        ...
        // REAL-TIME LISTENER FOR CHURCH EVENTS - IMPROVED VERSION
        const setupEventsListener = () => {
          console.log('🎯 Setting up real-time events listener...');
          
          // Primary collection name for events
          const primaryCollection = 'events';
          ...
          try {
            ...
            const eventsUnsubscribe = onSnapshot(
              eventsQuery, 
              (querySnapshot) => {
                console.log(`📡 Real-time events from ${primaryCollection}:`, querySnapshot.size, 'events');
                ...
                if (churchEvents.length > 0) {
                  console.log(`✅ Loaded ${churchEvents.length} events from ${primaryCollection}`);
                  ...
                  // Show notification for new events if this is not initial load
                  if (upcomingEvents.length > 0 && sortedEvents.length > upcomingEvents.length) {
                    const newEventsCount = sortedEvents.length - upcomingEvents.length;
                    toast({
                      title: 'New Events Available! 🎉',
                      description: `${newEventsCount} new church events have been posted.`,
                    });
                  }
                } else {
                  console.log(`📭 No upcoming events found in ${primaryCollection}`);
                  
                  // Optionally add sample events if no events exist
                  ...
                  // Uncomment the line below if you want to auto-add sample events when none exist
                  // addSampleEvents();
              ...
              (error) => {
                console.error(`❌ Events listener error for ${primaryCollection}:`, error);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <Card className="border-l-4 border-l-blue-500 shadow-lg">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Real-time from parish • Auto-updates</p>
            </CardContent>
          </Card>
        </div>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Live events from the parish • Updates automatically
                  </CardDescription>
                </div>
                <Button 
                  variant="outline" 
              ...
              <CardContent>
                <div className="space-y-4">
                  {upcomingEvents.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-xs mt-1">Real-time updates enabled</p>
                    </div>
                  ) : (
                ...
                <div className="mt-4 pt-4 border-t">
                  <p className="text-xs text-muted-foreground text-center">
                    🔄 Real-time updates active • Events sync automatically
                  </p>
                </div>
              </CardContent>
            </Card>
